...
def train(cfg, ac, envs, eval_envs, writer):
    ...
    num_envs = cfg.num_envs #10
    num_steps = cfg.num_steps #16
    ppo_batch_size = cfg.ppo_batch_size #160
    minibatch_size = int(cfg.ppo_batch_size // cfg.num_minibatches) #5

    global global_step
    best_metric = {"ssim": 0.9341586669}
    device = torch.device(cfg.device)
    kmask_shape = (envs.act_dim,) #{tuple: 1}(128,)
...
def prepare_train_envs(cfg, train_loader):
    ...
    print(cfg.env)
    envs = Brain_Env(train_loader, budget=cfg.budget, observation_space=observation_space, device=cfg.device)
    return envs


def prepare_evaluate_envs(cfg, val_loader):
    observation_space = cfg.env.observation_space
    envs = Brain_Env(val_loader, budget=cfg.budget, observation_space=observation_space, device=cfg.device)
    return envs
