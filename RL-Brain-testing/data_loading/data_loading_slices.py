...
class MRIDataset(Dataset):
    ...
    def load_data(self, base_root: pathlib.Path):
        ...
        for folder in patient_folders:
            ...
            slices = nib.load(label_path).shape[-1]

            # for slice_idx in range(47,49):
            for slice_idx in range(slices):
                self.examples.append((modality_path, slice_idx, label_path))
    ...
    def __getitem__(self, idx):
        ...
        """
        modality_path, slice_idx, path_label = self.examples[idx]
        ...
        label_data = nib.load(path_label).get_fdata()[:, :, slice_idx].astype(np.float32)
